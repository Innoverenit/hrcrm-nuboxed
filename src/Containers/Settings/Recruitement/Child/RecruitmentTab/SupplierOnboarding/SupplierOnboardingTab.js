...
class SupplierOnboardingTab extends Component {
  constructor(props) {
    super(props);
    this.formRef = null;
    this.state = {
  ...
  componentDidMount() {
      this.props.getProcessForSupplier(this.props.orgId);
  }
  handleTabChange = (key) => {
...
handleStagePublishClick = (supplierUnboardingStagesId, publishInd) => {
  ...
  handleCallback = (status) => {
    if (status === "success") {
       return getProcessForSupplier(this.props.orgId);
    } else {
      return null;
    }
  };
  handleAddWorkflow = () => {
    const { addProcessForSuppOnboarding, workflows } = this.props;
    const {
      workflowName,
      isTextInputOpen,
      orgId,
    ...
    if (exist) {
      message.error(
        ...
        addProcessForSuppOnboarding(header,  this.props.orgId, () => 
      this.handleCallback
      );
    }
  ...
  render() {
    const { addingProcessForSupplier, addProcessForSuppOnboarding } = this.props;
    return (
      <>
        <StageWrapper>
          <MainWrapper>
            ...
            <div class=" flex">
               <StyledTabs
                ...
                type="card"
              >
                {this.props.supplierProcess.map((item, i) => {
                  return (
                    <TabPane
                      key={i}
                      tab={
             ...
             >
                ...
                <Button
                  ...
                  loading={addingProcessForSupplier}
...
const mapStateToProps = ({ settings, auth }) => ({
    addingProcessForSupplier:settings.addingProcessForSupplier,
    addingProcessForSupplierError:settings.addingProcessForSupplierError,
    supplierProcess: settings.supplierProcess,
   fetchingProcessForSupplier:settings.fetchingProcessForSupplier,
  organization:
  auth.userDetails &&
...
export default connect(mapStateToProps, mapDispatchToProps)(SupplierOnboardingTab);
