...
import {getProspectSectorOppWonData} from "./DataRoomAction"
...
function ProspectQuotationSectorWonData(props) {
  ...
  useEffect(() => {
    if(props.selectedPropsectSector){
        setPage(page + 1);
    props.getProspectSectorOppWonData(
  ...
  const handleLoadMore = () => {
    const callPageMapd = props.prospectSectorOppWonTask && props.prospectSectorOppWonTask.length &&props.prospectSectorOppWonTask[0].pageCount
    setTimeout(() => {  
      if  (props.prospectSectorOppWonTask)
      {
...
if (props.fetchingProspectSectorOppWonTask) return <BundleLoader/>;
  ...
  {props.prospectSectorOppWonTask.length===0?<NodataFoundPage/>:    <InfiniteScroll
    dataLength={props.prospectSectorOppWonTask.length}
    next={handleLoadMore}
    hasMore={hasMore}
    loader={props.fetchingProspectSectorOppWonTask?<div class="flex justify-center">Loading...</div>:null}
    height={"83vh"}
  ...
  {props.prospectSectorOppWonTask.map((item) => {
    return (
...
const mapStateToProps = ({ customer,auth,datRoom }) => ({
  ...
  prospectSectorOppWonTask:datRoom.prospectSectorOppWonTask,
  fetchingProspectSectorOppWonTask:datRoom.fetchingProspectSectorOppWonTask,
  ...
  bindActionCreators(
    {
        ...
        getProspectSectorOppWonData,
     
    },
    dispatch
  );
export default connect(mapStateToProps, mapDispatchToProps)(ProspectQuotationSectorWonData);
