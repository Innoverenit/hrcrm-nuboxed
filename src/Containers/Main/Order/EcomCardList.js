...
import { getEcomList, handleItemViewDrawer } from "./OrderAction";
...
const EcomInvoiceListDrawer = lazy(() =>
  import("../Account/AccountDetailsTab/AccountOrderTab/EcomInvoiceListDrawer")
);
...
function EcomCardList(props) {
  ...
  useEffect(() => {
    setData(
      props.ecomList.map((item, index) => ({ ...item, key: String(index) }))
    );
  }, [props.ecomList]);
  ...
  const handleLoadMore = () => {
    const callPageMapd =
      props.ecomList && props.ecomList.length && props.ecomList[0].pageCount;
    setTimeout(() => {
      const { getEcomList } = props;
      if (props.ecomList) {
        ...
        <>
          <div class="rounded m-1 max-sm:m-1 p-1 w-[100%]  overflow-x-hidden shadow-[4px_0px_9px_3px_] shadow-[#a3abb980] bg-[#eaedf1] max-sm:hidden">
            ...
            <InfiniteScroll
              ...
              endMessage={
                <div class="flex font-poppins text-center font-bold text-xs text-red-500">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <div className=" flex items-center justify-center h-8 ml-gap  bg-[#eef2f9] w-[13.5rem]  max-md:w-[13.5rem] max-sm:flex-row  max-sm:justify-between  ">
                        <div class=" text-xs ml-gap  font-poppins"></div>
                      </div>
                      ...
                      <div className=" flex items-center justify-center h-8 ml-gap  bg-[#eef2f9] w-[3.1rem] max-md:w-[3.1rem] max-sm:flex-row  max-sm:justify-between  ">
                        <div
                          class=" text-xs cursor-pointer text-blue-500 font-poppins"
                          onClick={() => {
                      ...
                      <div className=" flex items-center justify-center h-8 ml-gap  bg-[#eef2f9] w-[7.8rem]  max-md:w-[7.8rem] max-sm:flex-row  max-sm:justify-between  ">
                        <div class=" text-xs   font-poppins">{item.status}</div>
                      </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(EcomCardList);
