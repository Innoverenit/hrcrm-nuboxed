...
import { getProductionUsersById, getRejectedPhonesByTechnician, assignRejectedPhones } from "./RefurbishAction"
import QRCodeModal from '../../../Components/UI/Elements/QRCodeModal'
import { SubTitle } from '../../../Components/UI/Elements';
import dayjs from "dayjs";

const { Option } = Select;
const RejectReassignForm = (props) => {
    ...
    useEffect(() => {
        ...
        props.getRejectedPhonesByTechnician(props.orderPhoneId, props.row.technicianId)
    ...
    return (
        <div>
            ...
            <StyledTable
                rowKey="phoneId"
                dataSource={props.rejectPhoneByUser}
                pagination={false}
                columns={column}
                rowSelection={rowSelection}
                loading={props.fetchingRejectedPhonesByTechnician}
            />
            <div class="flex justify-end mt-1">
                {department && technician && dueDate && checkedValue && <Button
                    type='primary'
                    loading={props.reassigningrejectedphone}
                    onClick={() => props.assignRejectedPhones({
...
const mapStateToProps = ({ auth, refurbish, departments }) => ({
    ...
    rejectPhoneByUser: refurbish.rejectPhoneByUser,
    noOfPhoneById: refurbish.noOfPhoneById,
    showAssignRepairModal: refurbish.showAssignRepairModal,
    locationId: auth.userDetails.locationId,
    fetchingRejectedPhonesByTechnician: refurbish.fetchingRejectedPhonesByTechnician,
    userId: auth.userDetails.userId,
    departments: departments.departments,
    reassigningrejectedphone: refurbish.reassigningrejectedphone
});
    ...
    bindActionCreators(
        {
            ...
            getRejectedPhonesByTechnician,
            assignRejectedPhones,
            getDepartments
        },
        dispatch
    );
