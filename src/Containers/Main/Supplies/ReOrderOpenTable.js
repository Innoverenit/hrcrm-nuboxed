...
function ReOrderOpenTable(props) {
      ...
      <div className=" flex sticky z-auto mt-4">
        <div class="rounded m-1 max-sm:m-1 p-1 w-full overflow-auto shadow-[4px_0px_9px_3px_] shadow-[#a3abb980] bg-[white]">
          <div className=" flex max-sm:hidden  w-[81%] justify-between  p-1 bg-transparent font-bold sticky items-end z-10">
            ...
            {props.reOrderData.length ?
              <>
                {props.reOrderData.map((item,index) => {
              ...
              !props.reOrderData.length
                && !props.fetchingReorderData ? <EmptyPage/> : null}
...
const mapStateToProps = ({ dashboard, auth,settings }) => ({
  fetchingReorderData: dashboard.fetchingReorderData,
...
export default connect(mapStateToProps, mapDispatchToProps)(ReOrderOpenTable);
