...
function OrderPaymentTable(props) {
  ...
  useEffect(() => {
    if (props.paymentModee && props.paymentModee.length > 0) {
      setpayModeOptions(props.paymentModee);
    }
  }, [props.paymentModee]);

  useEffect(() => {
    props.getDistributorOrderPayment(
                      ...
                      <div class=" text-xs  font-poppins">
                        {row.paymentId === item.paymentId && edit ? (
                          <Select
                            ...
                            onCLick={fetchPayModeOptions}
                            loading={loadingpayMode}
                          >
                            {payModeOptions.map((a) => {
                              return (
                                <Option value={a.paymentCatagoryId}>
                                  {a.name}
                                </Option>
                              );
                            })}
                        ...
                        {item.approveByFinanceInd === true ? (
                          <div class="flex items-center">
                            ...
                            <span class="text-green-700">
                              {" "}
                              {dayjs(item.approveDate).format("YYYY-MM-DD")}
                            </span>
                          </div>
                        ) : (
                          "None"
