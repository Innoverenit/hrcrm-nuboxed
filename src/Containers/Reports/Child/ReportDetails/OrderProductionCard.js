...
const OrderProductionCard = (props) => {
  ...
  const [loading1, setLoading1] = useState(false);
    ...
    const fetchData1 = async () => {
      ...
      try {
        const response = await axios.get(`${base_url2}/dashboard/production/${props.userId}/${props.startDate}/${props.endDate}/${page}`,{
          headers: {
    ...
    useEffect(() => {
      if (props.selectedCategory==="Orders" && props.selectedButtonIcon==="production"){
        fetchData1();
      }
...
<div class="flex  max-sm:justify-end max-sm:w-wk items-center">    
  ...
  const mapDispatchToProps = (dispatch) =>
    ...
    export default connect(mapStateToProps, mapDispatchToProps)(OrderProductionCard);
