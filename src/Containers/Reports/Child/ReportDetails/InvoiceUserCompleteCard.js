...
const InvoiceUserCompleteCard = (props) => {
  ...
  const [loading1, setLoading1] = useState(false);
   ...
   const [translatedMenuItems, setTranslatedMenuItems] = useState([]);
    ...
    const fetchData1 = async () => {
      ...
      try {
        const response = await axios.get(`${base_url2}/dashboard/completeInvoiceList/${props.userId}/${props.startDate}/${props.endDate}/${page}`,{
          headers: {
...
{item.priority === "High" && (
  ...
  const mapDispatchToProps = (dispatch) =>
    ...
    export default connect(mapStateToProps, mapDispatchToProps)(InvoiceUserCompleteCard);