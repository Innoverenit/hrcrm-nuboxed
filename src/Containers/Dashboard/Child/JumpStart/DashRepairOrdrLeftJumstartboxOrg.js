...
function DashRepairOrdrLeftJumstartboxOrg(props) {
  ...
  const [loading1, setLoading1] = useState(false);
    ...
    const fetchOrderInward = async () => {
      try {
        const response = await axios.get(`${base_url2}/phoneOrder/orderInward/${props.orgId}/${props.startDate}/${props.endDate}`,{
          headers: {
  ...
  const [loading3, setLoading3] = useState(false);
    const fetchQcApproved = async () => {
      try {
        const response = await axios.get(`${base_url2}/phoneOrder/qcApproved/${props.orgId}/${props.startDate}/${props.endDate}`,{
          headers: {
            Authorization: "Bearer " + sessionStorage.getItem("token") || "",
          },
        });
    ...
    const fetchRepairApproved = async () => {
      try {
        const response = await axios.get(`${base_url2}/phoneOrder/repairApproved/${props.orgId}/${props.startDate}/${props.endDate}`,{
          headers: {
            Authorization: "Bearer " + sessionStorage.getItem("token") || "",
          },
        });
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(DashRepairOrdrLeftJumstartboxOrg);
